...
const fs = require('fs');
const { program, ProgramOption, colors } = require('playwright-core/lib/utilsBundle');
const { registry } = require('playwright-core/lib/server');

// Get path to playwright internals
const playwrightPath = path.dirname(require.resolve('playwright/package.json'));
const mcpServer = require(path.join(playwrightPath, 'lib/mcp/sdk/server.js'));
const { resolveCLIConfig, commaSeparatedList, semicolonSeparatedList, headerParser, enumParser, numberParser, resolutionParser, dotenvFileLoader } = require(path.join(playwrightPath, 'lib/mcp/browser/config.js'));
const { setupExitWatchdog } = require(path.join(playwrightPath, 'lib/mcp/browser/watchdog.js'));
const { contextFactory } = require(path.join(playwrightPath, 'lib/mcp/browser/browserContextFactory.js'));
const { ExtensionContextFactory } = require(path.join(playwrightPath, 'lib/mcp/extension/extensionContextFactory.js'));

// Import our custom patched backend
const { createPatchedServerBackend } = require('./src/custom/index.js');
...
// Add all options (mirroring decorateCommand from playwright)
p.option('--allowed-hosts <hosts...>', 'comma-separated list of hosts this server is allowed to serve from. Defaults to the host the server is bound to. Pass \'*\' to disable the host check.', commaSeparatedList)
  ...
  .action(async (options) => {
    ...
    if (options.extension) {
      // For extension mode, use patched backend with extension factory
      const serverBackendFactory = {
        name: 'Playwright w/ extension',
        nameInConfig: 'playwright-extension',
        version: packageJSON.version,
        create: () => createPatchedServerBackend(config, extensionContextFactory)
      };
      await mcpServer.start(serverBackendFactory, config.server);
      return;
    }

    // Use our patched backend factory for standard mode
    const factory = {
      name: 'Playwright',
      nameInConfig: 'playwright',
      version: packageJSON.version,
      create: () => createPatchedServerBackend(config, browserContextFactory)
    };
